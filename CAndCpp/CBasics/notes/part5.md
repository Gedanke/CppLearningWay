...
数组属于构造数据类型，一个数组的元素可以是基本数据类型或构造类型
...
![](../photos/part5/%E5%86%92%E6%B3%A1%E6%8E%92%E5%BA%8F.png)
...
数组名是一个地址的常量，代表数组中首元素的地址，`数组的首地址 == 数组的首元素地址 == 数组的首行地址`
...
* 字符串一定是一个 `char` 的数组，但 `char` 数组未必是字符串
